...
os.system('touch %s' % speakers_fn)
speakers_data = yaml.load(open(speakers_fn)) or {}
...
for _name in data:
    ...
    if RATE16K_MONO in _name:
        speakers_data.setdefault(_name, '?')
...
with open(speakers_fn, 'w') as fout:
    yaml.dump(speakers_data, fout, default_flow_style=False)
