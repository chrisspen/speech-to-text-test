...
def load_model_en_jasper(model_weights, batch_norm_eps = 0.001, num_classes = 29, ABC = " ABCDEFGHIJKLMNOPQRSTUVWXYZ'|"):
    class JasperNet(nn.ModuleList):
        def __init__(self, num_classes):
            def conv_bn_residual(kernel_size, num_channels, stride = 1, dilation = 1, padding = 0, repeat = 1, num_channels_residual=None):
                ...
                return nn.ModuleDict(dict(
                    conv = nn.ModuleList([
                        ...
                        stride = stride, dilation = dilation, padding = padding) for i in range(repeat)]),
            ...
            blocks = nn.ModuleList([
                ...
                conv_bn_residual(
                    kernel_size = 17, num_channels = (384, 512), padding = 8, repeat = 5, num_channels_residual = [256, 256, 256, 384, 384]),
                conv_bn_residual(
                ...
                conv_bn_residual(
                    kernel_size = 25, num_channels = (640, 768), padding = 12, repeat = 5,
                    num_channels_residual = [256, 256, 256, 384, 384, 512, 512, 640, 640]),
                conv_bn_residual(
                    kernel_size = 25, num_channels = (768, 768), padding = 12, repeat = 5,
                    num_channels_residual = [256, 256, 256, 384, 384, 512, 512, 640, 640, 768]),
    ...
    def frontend(signal, sample_freq, window_size=20e-3, window_stride=10e-3, dither = 1e-5, window_fn = np.hanning, num_features = 64):
        ...
        S = torch.stft(
            ...
            window = torch.hann_window(int(window_size * sample_freq)).type_as(signal),
            pad_mode = 'reflect', center = True).pow(2).sum(dim = -1)
    ...
    def conv_block(kernel_size, num_channels, stride = 1, dilation = 1, repeat = 1, padding = 0):
        ...
        for i in range(repeat):
            modules.append(nn.Conv1d(
                ...
                num_channels[1], kernel_size = kernel_size, stride = stride, dilation = dilation, padding = padding))
...
class Tester(BaseTester):
    ...
    def __init__(self, *args, **kwargs):
        ...
        model = 'en_jasper'
        weights = '../data/models/jasper/jasper10x5_LibriSpeech_nvgrad_masks.h5'

        self.device = 'cpu'

        torch.set_grad_enabled(False)
        self.frontend, self.model, self.idx2chr, self.chr2idx = load_model_en_jasper(weights)
